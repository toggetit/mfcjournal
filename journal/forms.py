...
class NewInForm(forms.Form):
    rec_num = forms.CharField(label='№ входящего документа', widget=forms.TextInput(attrs={'class': 'form-control'}), max_length=50)
    rec_org = forms.ModelChoiceField(queryset=Org.objects.all(), widget=forms.Select(attrs={'class':'form-control'}), empty_label=None, label='Корреспондент')
    ...
    control_date = forms.DateField(label='Крайняя дата исполнения', widget=forms.DateInput(attrs={'class': 'form-control'}), required=False)
